...
from products.swaption.cpu import price_book_swaptions
...
def simulate_nested_cva(
    ...
    nested_cva = np.zeros(num_outer_paths, dtype=np.float32)
    squared_nested_cva = np.zeros(num_outer_paths, dtype=np.float32)

    for outer_path in range(num_outer_paths):
                    ...
                    if product_type == 'swap':
                        ...
                        for trade in irs:
                            ...
                            price = price_irs(
                                t=curr_t_left,
                                r=r_left,
                                r_last_fixing=fixing_rates_left[m],
    ...
    return nested_cva, squared_nested_cva


def simulate_nested_cva_swaptions(
    swaptions, X, default_step, rate_integral_path, gamma_integral_path,
    diff_params, num_outer_paths, num_steps_total, num_inner_ls_paths,
    dt, dT, N_b, rng, indicator_in_cva=False,
):
    a, b, sigma, k, theta, xi = diff_params
    fixing_window_size = rate_integral_path.shape[0] - num_steps_total

    nested_cva = np.zeros(num_outer_paths, dtype=np.float32)
    squared_nested_cva = np.zeros(num_outer_paths, dtype=np.float32)

    for outer_path in range(num_outer_paths):  # M_cva
        payoff = 0.0
        if indicator_in_cva:
            ...
            mtm = price_book_swaptions(
                t=t_lp1, r_0=float(r_at_default), swaptions=swaptions,
                M=num_inner_ls_paths, dt=dt, a=a, b=b, sigma=sigma, N_b=N_b, rng=rng,
            )
            ...
            for step in range(num_steps_total):
                ...
                mtm = price_book_swaptions(
                    t=t_l, r_0=float(r_at_l), swaptions=swaptions,
                    M=num_inner_ls_paths, dt=dt, a=a, b=b, sigma=sigma, N_b=N_b, rng=rng,
                )
