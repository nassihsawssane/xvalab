...
def vasicek_A(t, T, a, b, sigma):
    B = vasicek_B(t, T, a)
    return math.exp((b - 0.5 * sigma * sigma / (a * a)) * (B - (T - t))
                    - 0.25 * sigma * sigma / a * B * B)
...
def price_irs(t, r, r_last_fixing, first_reset, reset_freq, num_resets,
              swap_rate, a, b, sigma, only_fixed_leg, eps):
    maturity = first_reset + (num_resets - 1) * reset_freq
    if t > maturity + eps:
        return 0.0

    if t >= first_reset - eps:
        k = int((t - first_reset + eps) / reset_freq)
        if k < 0:
            k = 0
    else:
        k = -1
    ...
    for i in range(k + 1, num_resets):
        ...
        fixed_leg += zcb_last
    fixed_leg = fixed_leg * reset_freq * swap_rate
    ...
    if k == -1:
        ...
        if k == 0:
            ...
            inv_zcb_reset = 1.0 / price_zcb(0.0, reset_freq, r_last_fixing, a, b, sigma)
